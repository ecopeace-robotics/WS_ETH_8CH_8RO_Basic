...
| `.c` 파일 `static` 함수 | **필수에 준함** | 로직이 자명하지 않으면 `@brief` + `@param[in/out]` + `@return` |
| `.c` 파일 `static` 변수 | 조건부 권장 | 역할·초기값이 자명하지 않으면 `//` 또는 `/** */` 주석 |

---

> **내부 학습 문서의 관점에서:**
>
> 외부에 배포되는 라이브러리 문서라면 `static` 함수를 생략해도 된다.
> 그러나 **이 문서의 목적은 팀원이 구현 원리를 이해하는 것**이다.
> `static` 함수는 흔히 핵심 하드웨어 제어 로직, 프로토콜 파싱, 캐시 관리 등을 담는다.
> 이를 설명하지 않으면 `.c` 파일을 읽는 사람은 맥락 없이 코드를 해독해야 한다.
>
> **따라서:** 로직이 자명하지 않은 static 함수는 공용 API와 동일한 수준으로 문서화한다.
> — `@param[in]`, `@return` 등으로 호출 규약을 명확히 하고,
> 필요하면 `@details`에 처리 흐름을 설명한다.
...
`static` 함수는 헤더에 선언이 없으므로, 주석이 부족하면 함수를 처음 보는 팀원이 호출 규약을 이해하기 어렵다.
로직이 자명하지 않은 `static` 함수는 **공용 API와 동일한 수준으로 문서화**한다.

#### static 함수 주석 작성 기준

| 경우 | 작성 수준 |
|---|---|
| 이름만으로 역할이 명확한 단순 변환 함수 | `@brief` 1줄 |
| 레지스터·프로토콜·캐시 등을 다루는 함수 | `@brief` + `@param[in]` + `@return` |
| 분기·상태머신·이벤트 흐름이 있는 함수 | 위 + `@details` (필요 시 Graphviz) |

#### 코드 예시 (exio.c의 tca9554_write_reg 기반)
 ...
 * 내부적으로 ACK 확인까지 처리하며, 실패 시 ESP_LOGE로 기록한다.
 *
...
- **로깅 태그** (`static const char *TAG = "XXX"`): 역할이 자명하므로 `//` 한 줄로 충분
- **모듈 상태 변수** (`s_output_cache`, `s_eth_handle`, `s_pulse_us[]`):
  역할과 초기값 의미를 `// ───` 블록 주석으로 설명
  (Doxygen `/** */` 블록은 불필요)
